...
def print_death_age(people_dict):
    ...
    for key in people_dict:
        ...
        print(person[NAME_INDEX], person[DEATH_YEAR_INDEX] - person[BIRTH_YEAR_INDEX])        
...
def count_genders(people_dict):
    ...
    print("Number of males: ", gender_count["M"])
    print("Number of females: ", gender_count["F"])
...
def print_marriages(marriages_dict, people_dict):
    ...
    for key in marriages_dict:
        ...
        husband_age = marriage[WEDDING_YEAR_INDEX] - husband[BIRTH_YEAR_INDEX]
        wife_age = marriage[WEDDING_YEAR_INDEX] - wife[BIRTH_YEAR_INDEX]
        print(f"{husband[NAME_INDEX]} {husband_age} > \
{marriage[WEDDING_YEAR_INDEX]} < {wife[NAME_INDEX]} {wife_age}")


def count_marriages(marriages_dict, people_dict):
    """For each person in the marriages dictionary, print
    their name and total number of marriages.
    """
    ...
    for key in marriages_dict:
        husband = people_dict[marriages_dict[key][HUSBAND_KEY_INDEX]]
        husband_name = husband[NAME_INDEX]
        wife = people_dict[marriages_dict[key][WIFE_KEY_INDEX]]
        wife_name = wife[NAME_INDEX]
        if husband_name in marriage_count:
            marriage_count[husband_name] += 1
        else:
            marriage_count[husband_name] = 1
        if wife_name in marriage_count:
            marriage_count[wife_name] += 1
        else:
            marriage_count[wife_name] = 1

    for key, value in marriage_count.items():
        print(f"{key} was married {value} times")
